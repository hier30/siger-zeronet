...
@section('title', 'Peta Absorpsi CO₂')
@section('page-title', 'Peta Absorpsi CO₂')
@section('page-subtitle', 'Visualisasi choropleth absorpsi karbon per kecamatan')

@section('year-filter')
<form method="GET" action="{{ route('peta-absorpsi') }}" class="flex items-center gap-2">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Color scale for absorption choropleth
    function getAbsorpsiColor(val) {
        return val > 3500 ? '#006954' :
               val > 2500 ? '#10B981' :
               val > 1500 ? '#6EE7B7' :
                            '#D1FAE5';
    }

    function getAbsorpsiLabel(val) {
    ...
    infoControl.update = function (props) {
        if (props) {
            ...
            this._div.innerHTML = '<h4>Peta Absorpsi CO₂</h4><span style="color:#9CA3AF;">Hover pada kecamatan</span>';
        }
    };
    ...
    legend.onAdd = function () {
        ...
        let html = '<h4>Absorpsi CO₂ (ton)</h4>';
        for (let i = 0; i < grades.length; i++) {
            html += `<i style="background:${getAbsorpsiColor(grades[i] + 1)}"></i> ${labels[i]}<br>`;
        }
        div.innerHTML = html;
        return div;
    };
    ...
    // Fetch GeoJSON data
    fetch(`/api/peta-absorpsi?tahun={{ $tahun }}`)
        .then(res => res.json())
        .then(data => {
            geojsonLayer = L.geoJSON(data, {
                style: function (feature) {
                    return {
                        fillColor: getAbsorpsiColor(feature.properties.absorpsi_co2),
                        weight: 2,
                        opacity: 1,
                        color: '#FFFFFF',
                        fillOpacity: 0.75
